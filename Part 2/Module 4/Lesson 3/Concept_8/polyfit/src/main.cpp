...
int main() {
   ...
   */
  // YOUR CODE HERE
  auto fitted_curve_coeff = polyfit(xvals,yvals,3);
  ...
  for (double x = 0; x <= 20; ++x) {
     ...
     */
    //std::cout << "Evaluated Y values for given X values are " <<polyeval(fitted_curve_coeff,x) << std::endl; 
    std::cout << "values are [" << x << " ," << polyeval(fitted_curve_coeff,x) << "]" << std::endl;
  }
